...
const MedicineCard = ({ medicine }: any) => {
  ...
  useEffect(()=>{
    let medicinePresentInTheCart = cartItems.find((cartItem:any)=>cartItem.item === medicine._id)
    if(medicinePresentInTheCart){
      setQuantity(medicinePresentInTheCart.quantity);
    }
    console.log(cartItems);
  },[cartItems, quantity])


  const apiLogin = async (medicineId:string) => {
    try {
        const response = await axios.post(`${baseURL}/cart/add-to-cart`,{
          medicineId,
      ...
      <div className="bg-white dark:bg-gray-800 shadow-lg hover:shadow-2xl transition-shadow duration-300 rounded-xl overflow-hidden transform hover:scale-105 transition-transform duration-300">
        ...
        <div className="p-6">
          ...
          <div className="mt-6 flex justify-between items-center">
            ...
            {loggedIn && quantity === 0 && <button onClick={()=>{
              apiLogin(medicine._id);
            ...
            {loggedIn && quantity !== 0 && <button className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-5 py-2 rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-transform duration-200">
              + <span>{quantity}</span> -
            </button>}
          </div>
        </div>
